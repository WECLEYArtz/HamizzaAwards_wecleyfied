document.addEventListener('DOMContentLoaded', () => {
    ...
    const totalItems = 8;  // We have 8 images, totalItems is 8
    let currentIndex = 0;
    ...
    const updateCarousel = () => {
        ...
        dots[Math.floor(currentIndex / itemsPerSlide)].classList.add('active');  // Group 2 items per dot on desktop
    };
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', () => {
            ...
            currentIndex = parseInt(dot.getAttribute('data-index')) * (window.innerWidth <= 768 ? 1 : 2);  // 1 for mobile, 2 for desktop
            updateCarousel();
